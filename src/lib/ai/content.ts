...
// 把模型概率转成"基调锚点"（中文）：明确热门/冷门 + 强弱幅度，强制 AI 与概率一致。
function probContextZh(home: string, away: string, p: Win1x2): string {
  ...
  const mag =
    favP >= 55 || margin >= 28
      ? `${fav}是明显大热门，${und}处于明显劣势（${und}若取胜就是大冷门）`
      : favP >= 45 || margin >= 10
        ? `${fav}略占上风，${und}有一搏之力`
        : `双方势均力敌`;
  return `【模型预测胜平负概率】${home}胜 ${ph}%、平局 ${pd}%、${away}胜 ${pa}%。基调务必与此一致：${mag}。严禁把劣势一方写成热门或大众更看好的一方。`;
}
...
function probContextEn(home: string, away: string, p: Win1x2): string {
  ...
  const mag =
    favP >= 55 || margin >= 28
      ? `${fav} are clear favorites and ${und} are big underdogs (a ${und} win would be a real upset)`
      : favP >= 45 || margin >= 10
        ? `${fav} are slight favorites; ${und} have a fighting chance`
        : `it is an even, coin-flip matchup`;
  return `[Model chances] ${home} win ${ph}%, draw ${pd}%, ${away} win ${pa}%. Your framing MUST match this: ${mag}. Never present the underdog as the favorite or the crowd pick.`;
}
...
- 若提供了【模型预测胜平负概率】，前瞻的倾向性必须与之一致：明确点出谁是热门、谁是潜在冷门；绝不能把概率明显更低的一方写成热门或大众更看好的一方。
- 不要写免责声明（系统会另行追加）。`;
...
- 若提供了【模型预测胜平负概率】，热门/冷门判断必须与之一致：概率最高的一方是热门，概率明显更低的一方才是潜在冷门；绝不能说反。
...
${EN_RULES}
- If [Model chances] are provided, your lean MUST match them: name the favorite and the underdog correctly; never frame the lower-probability side as the favorite.`;
...
${EN_RULES}
- If [Model chances] are provided, the favorite/dark-horse call MUST match them: the highest-probability side is the favorite, the clearly-lower side is the dark horse. Never swap them.`;
