...
1. SOURCE: Use ONLY INPUT.angle and INPUT.assets[].desc. For an image asset, its desc is a factual description of the image; for an embed/tweet asset, its desc is the user's note. Do NOT invent facts, numbers, statistics, quotes, scores, dates, player details, or anything not present in the angle or the descs. Never mention "INPUT", the prompt, the model, or any internal mechanics.
...
绝对铁律——违反任意一条整篇作废：
1. 素材：只用 INPUT.angle 和 INPUT.assets[].desc。图片素材的 desc 是对该图的事实描述；推文/嵌入素材的 desc 是用户的备注。绝不编造角度或 desc 之外的事实、数字、统计、引语、比分、日期、球员细节。绝不在文中提及「INPUT」、提示词、模型或任何内部机制。
