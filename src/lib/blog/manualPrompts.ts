...
1. SOURCE: Your only sources are INPUT.angle, the IMAGES attached to this message (each labeled "Image for asset N" — look at them and describe only what you actually see), and INPUT.assets[].desc (embed/tweet assets you CANNOT see — rely on their desc). Do NOT invent facts, numbers, statistics, quotes, scores, dates, player details, or anything not present in the angle, the images, or the descs. Never mention "INPUT", the prompt, the model, or any internal mechanics.
...
绝对铁律——违反任意一条整篇作废：
1. 素材：你的来源只有 INPUT.angle、本消息附带的图片（每张标注 "Image for asset N"——看图、只写你实际看到的）、以及 INPUT.assets[].desc（推文/嵌入素材你看不到，只能靠它的 desc）。绝不编造角度/图片/desc 之外的事实、数字、统计、引语、比分、日期、球员细节。绝不在文中提及「INPUT」、提示词、模型或任何内部机制。
