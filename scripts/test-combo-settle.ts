/**
 * 集成验证组合预测结算：造一笔 2 腿组合 → 分别结算两场 → 校验"全中才算赢、倍率连乘发放"→ 清理。
 * 运行：npx tsx scripts/test-combo-settle.ts
 */
...
async function main(): Promise<void> {
  ...
  if (ms.length < 2) throw new Error("市场不足 2 个");
  ...
  // 造 2 腿组合：选两场主胜，投入 100 积分，连乘 3×3=9（余额 1000→900）
  ...
  console.log("已造 2 腿组合：两场主胜，投入 100，连乘 9");
  ...
  // 结算第二场（主胜）→ 全中 → 判赢，发放 100×9=900，余额 900→1800
