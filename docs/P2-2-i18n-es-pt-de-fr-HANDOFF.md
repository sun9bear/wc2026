...
> 已落地并提交（全绿、行为零变化、未对用户暴露）：**A0** 工单（§12）、**A1** 路由配置驱动重构（locales.ts，commit d86e1be）、**A2** 4 语 UI 字典（satisfies Dict，未挂 dicts，commit 7cd7c78）、**A6 国名表** 192 条（NATIONS 必填字段，commit 86180e7）、**A3 rules+calculator+forecast** 4 语 COPY（staged）。
...
- **✅ rules + calculator + forecast 页已 staged（2026-06-14）**：`COPY` 补 es/pt/de/fr——rules（小组排名/最佳第三名判据全文）、calculator（计算器 title/description/teamTitle/teamDesc 函数键）、forecast（出线/夺冠概率页 20 键）；均 tsc/eslint/vitest 净、合规词 0 命中。**forecast 的 Dataset JSON-LD（含并行会话的 `license` 字段）与 GEO 答案段/variableMeasured/note 内联三元未碰，留 A4**；**calculator 页 shareText/nm 内联三元同留 A4**（激活时一并清）。
