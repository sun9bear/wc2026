...
- **✅ rules + calculator + forecast + group/[letter] + forecast/best-thirds（5 页）已 staged（2026-06-14）**：`COPY` 补 es/pt/de/fr——rules（小组排名/最佳第三名判据全文）、calculator（计算器 title/description/teamTitle/teamDesc 函数键）、forecast（出线/夺冠概率页 20 键）、group/[letter]（按组着陆页 title/desc/h1/cta/lead 函数键 + cols 数组）、best-thirds（最佳第三名页 20 键含 method/Annex C 文案）；均 tsc/eslint/vitest 净、合规词 0 命中。**未碰的 locale 三元（留 A4）**：forecast 的 Dataset JSON-LD（并行会话 `license` 字段）+ GEO 答案段/variableMeasured/note；calculator 的 shareText/nm；group 的 name() 队名助手 + JsonLd + 规则链接；best-thirds 的 GEO 答案段 + JsonLd + mapH 对阵行。激活时一并清。
