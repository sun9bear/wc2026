...
| 日期 | 交付 | 备注 |
|---|---|---|
| 6/14 | **P2-1 双语可爬 URL + hreflang 全量上线**（feat/legal-pages `a987623`，部署生产 dpl_9iLcu58Yq6Ytmoae5WzbxiPqYg6y + 线上验证；tsc 净 / 144 vitest / next build 净） | 把站点从「单 URL + cookie 切语言」改成 **per-locale 可爬 URL**：EN 留根（零 301）、ZH 加 `/zh/` 前缀。**A2 实现**：`src/proxy.ts` 把 `/zh/*` rewrite 到无前缀路由 + 注入 `x-locale` 头；`getLocale()` 头优先（根永远 EN，内容由 URL 唯一决定，不再靠 cookie 切）。新 helper `localeHref/stripLocale`(`src/i18n/index.ts`)、`LocaleContext`(客户端 locale)、`localizedAlternates/selfUrl`(`src/lib/seo/canonical.ts`)。各页 self-canonical + reciprocal hreflang(`en`/`zh-Hans`/`x-default`→EN)、JSON-LD/OG/分享 URL locale 化、全站内链 `localeHref`。`sitemap.ts` 每页双 locale + `xhtml:link`(286 loc/858 alternates，fail-closed 保留)；`seo-compliance-scan.ts` 改扫**真实 `/zh` 路径**(+b–l/privacy/disclaimer + redirect-collapse 加固，防中文树假绿)；`indexnow.ts` 补 `/zh`(仅 Bing/Yandex)；`public/llms.txt` 加 zh 树；`/privacy` 补英文版（与 disclaimer/about 同等双语，消除 hreflang/内容矛盾）。**21-agent 本地化 + 17-agent 5 维对抗评审**（唯一 blocker=zh QR 双 `/zh` 前缀已修；BottomNav 高亮/privacy 矛盾/扫描器加固 3 follow-up 全修）。线上验证：`/zh/forecast` **不带 Accept-Language 即返回中文**+自指 canonical、reciprocal hreflang、compliance scan en+zh 全 0 雷词。完整 spec+决策见 [`docs/P2-1-i18n-hreflang-HANDOFF.md`](P2-1-i18n-hreflang-HANDOFF.md)。**待用户**：GSC 对重点 `/zh` 页单独请求收录（中文树是全新暴露面）。 |
...
13. 全站英文内容补全：**比赛页 AI 正文已英文化（6/13，Gemini Flash 回填中）**；剩 about/玩法静态正文仍仅中文 + 路由级 /en + hreflang 待做。→ **P2-1（双语可爬 URL + hreflang）完整 spec 见 [`docs/P2-1-i18n-hreflang-HANDOFF.md`](P2-1-i18n-hreflang-HANDOFF.md)，新会话冷启动接手；这是全盘最大 SEO 结构性赢点（解锁中文海外华人面），定位为窗口后投资，红线=干净做或不做。**
